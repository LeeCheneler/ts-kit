...
describe("test command", () => {
  ...
  it("runs jest", async () => {
    await createPackage({ name: "@temp/test-command", testSuite: true });
    const result = runTsKit("test", {
      cwd: getPackageDir("@temp/test-command"),
    });
    ...
    expect(result.stderrLines).toContain("PASS ./example.test.ts");
    expect(result.stderrLines).toContain("  ✓ should pass (2ms)");
    expect(result.stderrLines).toContain("Test Suites: 1 passed, 1 total");
    expect(result.stderrLines).toContain("Ran all test suites.");
  });

  it("runs forwards args onto jest", async () => {
    await createPackage({ name: "@temp/test-command", testSuite: false });
